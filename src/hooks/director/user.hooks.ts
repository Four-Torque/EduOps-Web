...
import {
  ...
  deleteDirectorUsers,
} from "@/services/director/user.service";
import { useDirectorUserStore } from "@/store/director/user.store";
import type { UserApprovalStatus, DirectorUserListResponse } from "@/types/director/user.types";

export const directorUserQueryKeys = {
  all:  ()                               => ["director-users"]                        as const,
  list: (tab: string, page: number)      => ["director-users", "list", tab, page]     as const,
};
...
export function useUpdateDirectorUserStatus() {
  ...
  return useMutation({
    mutationFn: ({ userId, status }: { userId: number; status: UserApprovalStatus }) =>
      updateDirectorUserStatus(userId, status),

    onMutate: async ({ userId, status }) => {
      await queryClient.cancelQueries({ queryKey: directorUserQueryKeys.all() });
