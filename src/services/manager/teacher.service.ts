...
export async function fetchTeachers(): Promise<TeacherListItem[]> {
  ...
  return [...activeRes.data.body.data, ...inactiveRes.data.body.data].map(toTeacherListItem);
}
...
export async function fetchTeacherDetail(id: string): Promise<TeacherDetail> {
  ...
  const [classRes, pendingSalaryRes, completeSalaryRes, attendanceRes] = await Promise.allSettled([
    apiClient.get<{ body: ApiPaginatedClass }>("/class", { params: { teacherId: id, limit: 100 } }),
    apiClient.get<{ body: ApiSalary[] }>("/salary", { params: { userId: id, status: "PENDING" } }),
    apiClient.get<{ body: ApiSalary[] }>("/salary", { params: { userId: id, status: "COMPLETE" } }),
    apiClient.get<{ body: ApiStaffAttendance[] }>("/staff-attendance", { params: { userId: id } }),
  ]);
  ...
  const salaries = [
    ...(pendingSalaryRes.status === "fulfilled" ? pendingSalaryRes.value.data.body : []),
    ...(completeSalaryRes.status === "fulfilled" ? completeSalaryRes.value.data.body : []),
  ];
...
export async function updateTeacher(
  ...
  const current = (await apiClient.get<{ body: ApiUser }>(`/user/${id}`)).data.body;
