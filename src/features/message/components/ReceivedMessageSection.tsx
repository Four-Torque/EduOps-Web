...
import { useReceivedMessages }   from "@/features/message/query";
...
export function ReceivedMessageSection() {
  ...
  const { data: messages = [], isLoading } = useReceivedMessages();
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-[#f5f6f8] border-b border-slate-200">
              <th className="px-4 py-2.5 text-[11.5px] font-semibold text-slate-500 text-left">보낸 사람</th>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex items-center gap-1.5">
                        <p className="text-[12.5px] font-medium text-slate-900">{msg.sender?.name}</p>
                        {!msg.isRead && (
