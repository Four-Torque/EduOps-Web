    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div ref={ref} className="bg-white rounded-lg shadow-xl w-[360px] overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
          <h2 className="text-[13px] font-semibold text-slate-800">받는 사람 선택</h2>
          <button onClick={closeContactModal} className="text-slate-400 hover:text-slate-600 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto max-h-[320px]">
          {isLoading ? (
            ...
            <p className="text-[12px] text-slate-400 text-center py-6">검색 결과가 없습니다.</p>
          ) : (
            groups.map((group) => (
              <div key={group.label}>
