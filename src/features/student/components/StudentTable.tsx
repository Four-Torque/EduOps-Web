...
} from "@/shared/constants/manager/student.constants";
import { useStudentStore, useStudentRegisterStore } from "../store";
...
export function StudentTable() {
  ...
  // const allChecked =
  //   items.length > 0 && items.every((s: Student) => selectedIds.includes(s.id));
            ...
            <tr className="border-y border-slate-200 bg-[#f5f6f8]">
              <th className="px-3.5 py-[9px] w-10">
                <input
                  ...
                  // checked={allChecked}
                  onChange={() =>
                    toggleSelectAll(items.map((s: Student) => s.id))
                  }
        ...
        {!isLoading && totalItems > 0 && (
          <div className="flex items-center justify-between px-3.5 py-3 border-t border-slate-100">
            <p className="text-[11.5px] text-slate-400">
              Showing {(page - 1) * 8 + 1} to {Math.min(page * 8, totalItems)}{" "}
