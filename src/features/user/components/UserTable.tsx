...
import {
  useDirectorUsers,
  useUpdateDirectorUserStatus,
  useDeleteDirectorUsers,
} from "@/features/user/query";
import { useDirectorUserStore } from "@/features/user/store";
import { Pagination } from "@/shared/components/Pagination";
import { USER_TABLE_COLUMNS } from "@/shared/constants/director/user.constants";
import { DirectorUser, UserApprovalStatus } from "@/features/user/type";
...
export function UserTable() {
  ...
  const allChecked =
    items.length > 0 &&
    items.every((item: DirectorUser) => selectedIds.includes(item.id));
            ...
            <tr className="border-b border-slate-200 bg-[#f5f6f8]">
              <th className="px-3.5 py-[9px] w-10">
                <input
                  ...
                  checked={allChecked}
                  onChange={() =>
                    toggleSelectAll(items.map((item: DirectorUser) => item.id))
                  }
            ...
            {isLoading ? (
              ...
              items.map((user: DirectorUser) => (
                ...
                >
                  ...
                  <td className="px-3.5 py-2.5 text-[12.5px] text-slate-600">
                    {user.phone}
                  </td>
