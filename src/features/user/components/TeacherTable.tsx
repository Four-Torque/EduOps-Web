...
import { TeacherStatus, TeacherListItem } from "../type";
      ...
      <table className="w-full table-fixed border-collapse">
        ...
        <thead>
          <tr className="border-y border-slate-200 bg-[#f5f6f8] text-left">
            <th className="px-3.5 py-[9px] text-[11.5px] font-semibold text-slate-500">
              강사
            </th>
            <th className="px-3.5 py-[9px] text-[11.5px] font-semibold text-slate-500">
              이메일
        ...
        <tbody>
          {isLoading ? (
            ...
            <tr>
              <td
                colSpan={TOTAL_COLUMNS}
                className="px-3.5 py-8 text-center text-[12px] text-red-500"
              >
                목록을 불러오지 못했습니다.
              </td>
            </tr>
            ...
            teachers.map((teacher) => (
              ...
              >
                <td className="h-[45px] truncate overflow-hidden px-3.5 align-middle text-[12.5px] font-medium text-slate-900">
                  {teacher.name}
                </td>
                <td className="h-[45px] truncate overflow-hidden px-3.5 align-middle text-[12px] text-slate-600">
                  {teacher.email}
                </td>
                <td className="h-[45px] overflow-hidden px-3.5 align-middle text-[12px] text-slate-600">
                  {teacher.phone}
                </td>
                <td className="h-[45px] overflow-hidden px-3.5 align-middle text-[12px] text-slate-600">
                ...
                <td className="h-[45px] overflow-hidden px-3.5 align-middle text-[12px] text-slate-500">
                  {teacher.hireDate}
                </td>
          ...
          {!isLoading &&
            ...
            Array.from({ length: pageSize - teachers.length }, (_, i) => (
              <tr key={`empty-${i}`} className="h-[45px]">
                <td
                  colSpan={TOTAL_COLUMNS}
                  className="h-[45px] overflow-hidden px-3.5 align-middle text-[12px]"
                >
                  &nbsp;
                </td>
              </tr>
