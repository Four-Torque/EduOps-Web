...
export function CourseManagerDashboard() {
  ...
  const { data: syllabusData, isLoading: syllabusLoading } = useSyllabuses(1, 100);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex flex-col gap-6">
        ...
        <TabsContent value="syllabuses" className="m-0 focus-visible:outline-none flex flex-col gap-4">
          <SyllabusListTable syllabuses={syllabusData?.data || []} isLoading={syllabusLoading} isManager={true} />
        </TabsContent>
      </Tabs>
    </div>
  );
